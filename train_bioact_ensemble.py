...
def _train_one(X_train, y_train, seed: int):
...
def _loo_ensemble_predict(X, y, M=M, seeds=SEEDS):
    ...
    for i in range(n):
        ...
        for k, seed in enumerate(seeds[:M]):
            ...
            mm.fit(X_tr[sub], y_tr[sub], verbose=False)
...
def main():
    ...
    loo_means, loo_sigmas = _loo_ensemble_predict(X, y, M=M)
